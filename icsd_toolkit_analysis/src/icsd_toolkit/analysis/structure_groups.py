...
class StructureGrouper:
    ...
    def _group_structures(
        ...
        for cs, struct_in_cs in structures.items():
            ...
            icsd_groups = [
                sorted([structure._icsd_id for structure in group])
                for group in groups
            ]
    ...
    def group_structures(
        ...
        unique_docs = []
        docs_by_icsd_id = {
            doc.icsd_id: doc for doc in structure_docs
        }
    ...
    def group_structures_from_dataframe(self, dataframe : pd.DataFrame, **kwargs) -> list[IcsdStructureDoc]:
